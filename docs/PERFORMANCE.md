...
*   **Resolution Cap:** Captured video canvas elements in [`CameraScanner.jsx`](../frontend/src/components/scanner/CameraScanner.jsx) are capped at a maximum width of `1920px` and height of `1080px`.
...
*   **Implementation:** [`backend/routers/medication.py`](../backend/routers/medication.py) uses `asyncio.gather` to execute HTTP calls in parallel. This halves the total latency compared to sequential requests.
