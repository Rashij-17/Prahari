...
| **FastAPI Route Flooding** | Resource Exhaustion (Backend crash) | Flooding the `/scan/process` (OCR pipeline) endpoint with heavy base64 buffers. | **Max Payload Constraints:** FastAPI route dependencies inspect the incoming content-length headers and terminate payloads $> 5\text{MB}$ before processing. |
...
The backend environment configurations load keys at startup using standard `os.getenv` hooks. If keys are missing, the routes gracefully degrade into mock preview modes to prevent app crashes:
...
# Safety check: Prevent booting in production with missing keys
if not INFERMEDICA_APP_KEY and not os.getenv("DEBUG"):
    raise RuntimeError("CRITICAL: Infermedica keys missing in production environment!")
```
