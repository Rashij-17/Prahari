...
async def pocket_clinician_chat(
    ...
    common_test_drugs = ["ibuprofen", "aspirin", "nitroglycerin", "sildenafil", "amoxicillin", "cephalexin", "metformin", "pseudoephedrine", "atorvastatin", "spironolactone", "lisinopril", "warfarin", "allopurinol", "methotrexate"]
    ...
    # Run AI scan if:
    # 1. User clicked "Detailed AI Scan" (body.run_ai_scan is True)
    # OR 2. There are no local warnings (it's safe to run AI directly to answer)
    should_run_ai = (body.run_ai_scan or not local_warnings)
    ...
    if should_run_ai and is_gemini_configured:
        try:
            ...
            system_instruction = (
                ...
                "Guidelines:\n"
                "- Keep explanations simple, direct, and easily understandable by patients.\n"
                "- NEVER start your response with a greeting, hello, introduction, or generic opening like 'Hello. I am Prahari's Pocket Clinician' or 'I have analyzed your request...'. Start directly with the safety analysis or answer.\n"
                ...
                "- Always include a standard clinical disclaimer at the very end of your response stating that this is informational advice and they should consult a doctor.\n"
                "- Do NOT append any signature line or 'Response generated by' text at the end."
            )
            ...
            for msg in body.history[-6:]:  # Only keep last 6 turns
                ...
                # Clean up history messages to prevent prompt pollution and mimicking repetitive patterns
                cleaned_text = msg.text
                # Remove common signatures
                cleaned_text = cleaned_text.replace("✨ Response generated by Prahari Clinical AI.*", "")
                cleaned_text = cleaned_text.replace("✨ Response generated by Prahari Clinical AI.", "")
                cleaned_text = cleaned_text.replace("*✨ Response generated by Prahari Clinical AI.*", "")
                # Remove robotic greetings if present
                if cleaned_text.startswith("Hello. I am Prahari's Pocket Clinician."):
                    cleaned_text = cleaned_text.replace("Hello. I am Prahari's Pocket Clinician.", "").strip()
                if cleaned_text.startswith("Hello! I am Prahari’s Pocket Clinician."):
                    cleaned_text = cleaned_text.replace("Hello! I am Prahari’s Pocket Clinician.", "").strip()
                if cleaned_text.startswith("Hello! I am Prahari's Pocket Clinician."):
                    cleaned_text = cleaned_text.replace("Hello! I am Prahari's Pocket Clinician.", "").strip()
            ...
            # Clean up the output in case the model ignored constraints and generated signature or greeting anyway
            ai_text = ai_text.replace("✨ Response generated by Prahari Clinical AI.*", "")
            ai_text = ai_text.replace("✨ Response generated by Prahari Clinical AI.", "")
            ai_text = ai_text.replace("*✨ Response generated by Prahari Clinical AI.*", "")
            
            # Clean leading/trailing quotes or whitespace if present
            ai_text = ai_text.strip()
            
            # Append signature cleanly at the very end
            ai_text += "\n\n*✨ Response generated by Prahari Clinical AI.*"
    ...
    if local_warnings:
        warnings_desc = "\n".join([f"- ⚠️ **{w['matched_value']}**: {w['warning_text']}" for w in local_warnings])
        response_text = (
            f"I have evaluated your query against your active patient profile and found the following safety warnings:\n\n"
            f"{warnings_desc}\n\n"
            f"*Disclaimer: Always consult your healthcare provider or pharmacist before modifying your medicine intake.*"
        )
    else:
        response_text = (
            f"I have evaluated your query against your active patient profile, and no matching safety conflicts or allergy warnings were found.\n\n"
            f"To run a more detailed analysis, please click the **'Detailed AI Scan'** button below.\n\n"
            f"*Disclaimer: Always consult your healthcare provider or pharmacist before modifying your medicine intake.*"
        )
